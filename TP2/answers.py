...
def answer_01(data):
    ...
    df_scaled = data
    df_scaled.iloc[:,:-1] = ((data.iloc[:,:-1] -data.iloc[:,:-1].mean()) / data.iloc[:,:-1].std())
    return df_scaled
...
def answer_08():
    ...
    overfitting_models = ["RBF SVM", "Gaussian Process", "model", "Decision Tree"]
...
def answer_09():
    ...
    underfitting_models = ["Nearest Neighbors", "AdaBoost"]
